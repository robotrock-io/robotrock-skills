# OpenUI Lang for RobotRock chat

RobotRock dashboard GenUI follows the **[OpenUI](https://www.openui.com) spec**
([OpenUI Lang](https://www.openui.com/docs/openui-lang)).

**Required for visible data:** after tools that return lists, tables, search
rows, metrics, or similar, emit OpenUI Lang in a ```openui fence in the **same
turn**. Without it, the user only sees a Working / Used N tools trail — no
data card. There are no JSON “tool result” checkmark bars.

Prose (delta / policy) stays outside the fence as normal markdown.

```openui
root = Card([header, table])
header = CardHeader("Products", "Top matches")
table = Table([Col("Name", "string"), Col("Price", "number")], [["Keyboard", 79.99]])
```

Rules for RobotRock:

- Always start the program with `root = Card(...)`.
- Prefer one-column layouts suitable for a narrow chat rail.
- Inline tool result data into the OpenUI Lang (do not invent Query/Mutation tools).
- Do not repeat fields shown in the OpenUI card in your prose — add only the delta.
- Do not mention OpenUI or fences in user-facing prose.
- Prefer **one** OpenUI card per turn (primary result). Intermediate tools stay trail-only.
- External links: `Button("Open", { type: "open_url", url: "https://..." })`.
...
Image(src?: string, alt?: string, storageId?: string) — Displays an image. Prefer storageId from generate_image for durable chat images; src is the signed URL for first paint.
ImageBlock(src?: string, alt?: string, caption?: string, storageId?: string) — Image with optional caption. Prefer storageId from generate_image for durable chat images.
